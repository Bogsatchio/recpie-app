...
def get_recommender_engine():
    ...
    if recommender_engine is None:
        recommender_engine = RecommenderEngine(qd_client, get_recipe_repository())
    return recommender_engine


def create_app(recipe_repo=None, rec_engine=None) -> FastAPI:
    api = FastAPI()
    api.mount("/static", StaticFiles(directory=str(BASE_DIR / "static")), name="static")

    def repo():
        return recipe_repo or get_recipe_repository()

    def recommender():
        return rec_engine or get_recommender_engine()
    ...
    def query_by_ingredients(
        ...
        results = recommender().find_recipe_by_ingredients(
            ingredients,
            k,
    ...
    def query_by_name(
        ...
        results = recommender().find_recipe_by_name(
            name,
            k,
            category.value if category else None,
            cuisine.value if cuisine else None,
    ...
    def ingredient_suggestions(
        ...
        suggestions = recommender().get_suggestions(query, limit=fetch_limit)
        if exclude_items:
    ...
    def add_recipe(recipe: RecipeCreate, db: Session = Depends(get_db)):
        try:
            recipe_id = repo().insert_recipe(db, recipe)
            recommender().upsert_embedding(recipe_id=recipe_id, recipe=recipe)
            return {
    ...
    def update_recipe(recipe_id: int, recipe_update: RecipeUpdate, db: Session = Depends(get_db)):
        ...
        """
        try:
            existing_recipe = repo().get_recipe_by_id(db, recipe_id)
            ...
            rows_updated = repo().update_recipe(db, recipe_id, update_dict)
            if rows_updated == 0:
                raise HTTPException(status_code=500, detail="Failed to update recipe")

            try:
                recommender().upsert_embedding(recipe_id=recipe_id, recipe=merged_recipe)
            except Exception as qdrant_error:
                print(f"Warning: Failed to update recipe {recipe_id} in Qdrant: {str(qdrant_error)}")

            updated_recipe = repo().get_recipe_by_id(db, recipe_id)
    ...
    def delete_recipe(recipe_id: int, db: Session = Depends(get_db)):
        ...
        """
        try:
            rows_deleted = repo().delete_recipe(db, recipe_id)

            if rows_deleted == 0:
                raise HTTPException(status_code=404, detail=f"Recipe with id {recipe_id} not found")

            try:
                recommender().remove_recipe_from_indexes(recipe_id)
            except Exception as qdrant_error:
